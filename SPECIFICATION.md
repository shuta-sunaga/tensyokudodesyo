...
- キーワード検索
- 職種フィルター
- 人気キーワードタグ（キーワード検索と同一ロジック、シングルセレクト）
- ページネーション（10件/ページ）
- 都道府県別インタビュー表示
- 都道府県別企業表示
...
**検索フィルター**:
- **キーワード**: タイトル・企業名・キーワード・条件で検索（`job.title`, `job.company`, `job.keywords`, `job.conditions` を対象）
...
- **人気キーワードタグ**: 未経験歓迎（検索ワード:未経験）/土日祝休み/リモートワーク/高年収/研修充実（シングルセレクト、内部的にキーワード検索と同一ロジックで絞り込み）

**旧仕様（こだわり条件チェックボックス）**: チェックボックス＋条件IDマッピング方式で`job.conditions`をフィルタリングしていた。人気キーワードタグ方式に変更済み（2026-04-02）。
...
タグはシングルセレクト（1つだけ有効）。クリックでトグル。キーワード入力・職種セレクトとはAND条件で併用可能。検索対象は `job.title`, `job.company`, `job.keywords`, `job.conditions` の結合テキスト。

### 旧こだわり条件マッピング（廃止済み・2026-04-02）

以下は旧仕様。チェックボックス＋条件IDマッピング方式。復元が必要な場合はgit履歴から戻すこと。
