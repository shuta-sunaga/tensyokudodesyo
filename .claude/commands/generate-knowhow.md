...
`$ARGUMENTS` にユーザーからの補足・要望が渡される場合がある。例: `/generate-knowhow 30代向けに、AI活用のテーマで。実例多めで`

- **空の場合**: 通常どおり、カテゴリバランスに基づいてテーマを自動提案する
- **指定がある場合**: その内容を最優先で反映する。代表的な指定とその扱い:
  - **テーマ・トピック指定**（例:「AI活用」「面接対策で」）→ Stage 1 のテーマ提案を指定内容に沿って絞り込む
  - **ターゲット読者指定**（例:「30代向け」「未経験者向け」）→ Stage 2 本文のトーン・具体例を読者像に合わせる
  - **カテゴリ指定**（例:「履歴書の書き方カテゴリで」）→ Stage 1 で該当カテゴリに固定
  - **構成・内容の要望**（例:「実例多め」「チェックリスト必須」「文字数多め」）→ Stage 2 の品質基準に上乗せして反映する
  - **タイトル指定**（例:「タイトルは『〜』で」）→ Stage 1 のカニバリチェックは行うが、指定タイトルを軸に提案する
- 指定があっても **カニバリチェック（既存記事との重複確認）は必ず実施**し、重複リスクがある場合はその旨を提示して確認する
- `$ARGUMENTS` の内容が曖昧で複数解釈できる場合のみ、Stage 1 の停止時に合わせて確認する
...
0. **`$ARGUMENTS` を確認**し、補足・要望があれば「ユーザー補足・要望」セクションの方針に従って以降の提案に反映する
1. `knowhow.json` と `knowhow-mt.json` の全記事を読み込み、タイトル・カテゴリを把握
2. カテゴリバランスを分析し、手薄なカテゴリを特定（`$ARGUMENTS` でテーマ/カテゴリ指定がある場合はそれを優先）
