...
- **職種グループ**（`JobTaxonomy.BUCKETS`、2026-09-15 に Lark Base「職種（大分類）」の主要ラベル＝マイナビ転職系の大分類に合わせて再定義）: 製造・生産・品質管理 / 技術職（電気・電子・機械・半導体）/ 技術職（医薬・化学・素材・食品）/ 技術職（建築・土木・プラント・設備）/ 技能工・設備・配送・農林水産 / 営業職 / 事務・管理部門職 / ITエンジニア / 販売・サービス職 / 医療・福祉・介護 / 専門職・その他。Base の大分類・中分類・小分類はいずれもテキスト入力（中分類 821 種）で選択肢定義は無いため、表記ゆれは正規表現で吸収する。本番 4 県 + Base 4,021 件で検証し「その他」は 0.2% 以下。旧 ID `transport` は `skilled` に別名対応
...
```nginx
    ...
    # /mt/ は除外必須（正規表現 location が prefix の /mt/mt-static より優先され、管理画面の CSS/JS が 404 になる）
...
4. `nginx-patch-portal.py --apply`: portal.conf に expires / gzip / error_page 404（バックアップ `portal.conf.bak.20260915_025957`）。**事故**: 正規表現 location が `/mt/mt-static` の prefix location より優先され MT 管理画面の CSS/JS が 404 → 13:10 に `^/(?!mt/)` の除外と `^~ /mt/mt-static` で修正
